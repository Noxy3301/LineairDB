...
namespace {
// ---------------------------------------------------------------------------
// Typed cell codec (M2). Parses the proxy's ASCII val_str into a fixed-width
// LE binary at scatter time, and reformats the binary back into the *exact*
...
// renders a different length would corrupt the row (the md5 gates enforce it).
// Any parse/range failure returns false so the caller takes the heap fallback
// (never wrong, just unaccelerated) — exactly like an over-wide UNTYPED cell.
// ---------------------------------------------------------------------------
...
// exact — no double, so no rounding trap here (the compare-side trap is handled
// in the server, not the store).
...
// Parse one field's ASCII into `width` LE bytes returned as the low bytes of
// *out. Returns false on any failure (caller -> heap fallback).
inline bool ParseTyped(uint8_t kind, int scale, const std::byte* payload,
                       uint32_t len, uint32_t width, uint64_t* out) {
  (void)width;  // width is implied by kind (4/8); range checks are per-kind
...
void FormatTyped(uint8_t kind, int scale, const std::byte* cell, uint32_t width,
  ...
  switch (kind) {
    ...
    case FK_DATE: {
      ...
      int n = std::snprintf(buf, sizeof(buf), "%04lld-%02lld-%02lld",
...
// Proxy row format constants (proxy/lineairdb_field.hh):
// each field = [byteSize:1][valueLength:byteSize bytes LE][value bytes],
// where byteSize == 0xFF is a 1-byte field meaning valueLength == 0.
constexpr std::byte kNoValue{0xFF};

inline uint32_t LengthPrefixBytes(uint32_t len) {
  // Minimum base-256 digit count (mirrors
  // LineairDBField::calculate_minimum_byte_size_required).
  uint32_t n = 0;
...
// Parse the proxy row format into per-field (ptr, len) refs.
// Returns the number of fields parsed, or SIZE_MAX on malformed input /
// more fields than `max_fields`.
...
bool PaxGroup::ScatterRow(uint32_t slot, const std::byte* row, size_t size) {
  ...
  // Stack refs: TPC-C/TPC-H tables are far below this bound; wider tables
  // simply take the heap fallback.
  ...
  // Pass 1: validate. UNTYPED fields must fit their cell width; typed non-null
  // fields are parsed into a fixed-width LE binary scratch (any failure => heap
  // fallback, slot untouched — the write_counter is only bumped once every
  // field has validated).
  ...
  for (size_t f = 0; f < fields; f++) {
    ...
    if (k == FK_UNTYPED) {
      ...
      if (!ParseTyped(k, schema_.scale_of(f), refs[f].payload, refs[f].len,
                      schema_.field_max_bytes[f], &typed_bin[f]))
        return false;
    }
  }
...
namespace {
inline void AppendField(std::string& out, std::string_view payload) {
  if (payload.empty()) {
...
// Append field `f`'s proxy-format value: verbatim for UNTYPED, reformatted to
// the exact val_str ASCII for a typed present cell (empty => NULL field).
...
void PaxGroup::GatherRowSparse(uint32_t slot, const uint32_t* columns,
                               size_t n_columns, std::string& out) const {
  const size_t fields = schema_.field_count();
  AppendCellField(0, slot, out);  // null-flags field, always real
  size_t ci = 0;
  for (size_t f = 1; f < fields; f++) {
    if (ci < n_columns && static_cast<size_t>(columns[ci]) + 1 == f) {
      AppendCellField(static_cast<uint32_t>(f), slot, out);
      ci++;
    } else {
