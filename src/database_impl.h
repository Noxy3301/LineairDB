...
class Database::Impl {
 ...
 public:
   ...
   *   9. Install
   *        write payloads via `DataItem::Reset` and apply SI add/remove via
   *        `AddSecondaryIndexValue` / `RemoveSecondaryIndexValue`.
   *  10. Log snapshot
  ...
  bool ValidateAndCommit(
    ...
    // Step 9: install row writes/deletes and SI add/remove.
    ...
    // Install secondary-index add/remove operations
    for (auto& op : resolved_si_ops) {
      ...
      if (op.is_delete) {
        ...
        if (op.item->primary_keys().empty()) {
          op.index->Delete(op.secondary_key);
        }
      } else {
